...
**Status:** ✅ Profile complete. Ready to build.
...
systemd multi-user.target
    └── arch-installer.service       (iso/airootfs/etc/systemd/system/)
            └── arch-installer-session   (iso/airootfs/usr/local/bin/)
                    ...
                    ├── waits up to 10s for X to be ready
...
getty@tty1.service is masked to avoid conflict.
...
# Build the ISO (run from repo root)
sudo ./iso/build.sh

# Clean rebuild
sudo ./iso/build.sh --clean

# Build + immediately test in QEMU
sudo ./iso/build.sh --vm-test

# Build + copy ISO to NFS share automatically
sudo NFS_OUTPUT_DIR=/mnt/nas/isos ./iso/build.sh
```

Output ISO: `iso/out/arch-installer-YYYY.MM.DD-x86_64.iso`

### Key ISO files

| File | Purpose |
|------|---------|
| `iso/build.sh` | Main build + test wrapper around mkarchiso |
| `iso/profiledef.sh` | ISO metadata, bootmodes (BIOS+UEFI), squashfs compression |
| `iso/packages.x86_64` | All packages baked in: Python, GTK3, WebKit2GTK, Xorg, disk tools |
| `iso/pacman.conf` | Pacman config used during ISO build |
| `iso/airootfs/etc/customize_airootfs.sh` | Runs in chroot at build time — enables service, sets locale |
| `iso/airootfs/etc/systemd/system/arch-installer.service` | Systemd unit that starts the installer session |
| `iso/airootfs/usr/local/bin/arch-installer-session` | Session script: starts X, waits, launches installer |
| `iso/airootfs/etc/X11/xorg.conf.d/10-arch-installer.conf` | Minimal Xorg config (works in VMs without config) |
| `iso/efiboot/loader/entries/arch-installer.conf` | UEFI boot entry (systemd-boot) |
| `iso/efiboot/loader/entries/arch-installer-debug.conf` | UEFI debug entry (verbose, no autostart) |
| `iso/syslinux/syslinux.cfg` | BIOS boot menu |

### Boot entries
- **Normal** — quiet boot, autostart installer
- **Debug** — verbose, drops to TTY (for troubleshooting the live environment)

### cow_spacesize
Both boot entries pass `cow_spacesize=2G` — this is the overlayfs write layer.
...
| Build fails (mount errors) | `sudo findmnt \| grep work` → `sudo umount -R iso/work` |
...
- [ ] Full end-to-end install test not yet performed (custom ISO resolves the
      Python version mismatch that was blocking this)
...
| 14      | docs: update CLAUDE.md — ISO profile complete, "Next Phase" → done           |
